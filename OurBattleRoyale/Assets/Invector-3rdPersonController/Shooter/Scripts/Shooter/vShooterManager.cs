    ...
    public class vShooterManager : vMonoBehaviour
    {
        ...
        protected virtual void UpdateAmmoDisplay(int displayId)
        {
            ...
            if (useAmmoDisplay && ammoDisplay)
            {
                ...
                {
                    string display1 = weapon.autoReload ? weapon.isInfinityAmmo ? "Infinity" : (weapon.ammoCount + totalAmmo).ToString() : (string.Format("{0} / {1}", weapon.ammoCount, (weapon.isInfinityAmmo ? "Infinity" : totalAmmo.ToString())));
                    string displat2 = weapon.secundaryWeapon.autoReload ? weapon.secundaryWeapon.isInfinityAmmo ? "Infinity" : (weapon.secundaryWeapon.ammoCount + secundaryTotalAmmo).ToString() : (string.Format("{0} / {1}", weapon.secundaryWeapon.ammoCount, (weapon.secundaryWeapon.isInfinityAmmo ? "Infinity" : secundaryTotalAmmo.ToString())));
                    ammoDisplay.UpdateDisplay(display1 + "\n" + displat2, weapon.ammoID);
                }
                else
                {
                    string text = weapon.autoReload ? weapon.isInfinityAmmo?"Infinity": (weapon.ammoCount + totalAmmo).ToString() : (string.Format("{0} / {1}", weapon.ammoCount,(weapon.isInfinityAmmo?"Infinity": totalAmmo.ToString())));

...
public class v_car_randomLight : MonoBehaviour
{
    ...
    void Start()
    {
        originalColor = GetComponent<LensFlare>().color;
        originalIntensity = GetComponent<LensFlare>().brightness;
    }
    ...
    void Update()
    {
       LensFlare light = GetComponent<LensFlare>();
        if (affectsIntensity)
            light.brightness = originalIntensity * EvalWave();
        Color o = originalColor;
        Color c = GetComponent<LensFlare>().color;

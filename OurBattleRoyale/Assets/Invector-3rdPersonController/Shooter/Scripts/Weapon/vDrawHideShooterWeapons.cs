...
namespace Invector.vShooter
{
    ...
    public class vDrawHideShooterWeapons : vDrawHideMeleeWeapons
    {
        ...
        {
            return (shooter && shooter.shooterManager && shooter.shooterManager.CurrentWeapon && (forceHide || (!shooter.isAiming && shooter.aimTimming<=0 && !shooter.isReloading)))
                    || (base.CanHideWeapons() && (forceHide || (!shooter.isAiming && shooter.aimTimming <= 0 && !shooter.isReloading)));
        }
        ...
        {
            if (shooter && shooter.shooterManager && !shooter.cc.customAction && shooter.shooterManager.CurrentWeapon && shooter.CurrentActiveWeapon == null && !shooter.isAiming && !shooter.shooterManager.hipfireShot && shooter.shotInput.GetButtonDown())
            {
                if (!IsEquipping)
                {
